...
function readAll(){
    ...
    let temp2 = [] 
    //读取文本全部内容
    fsPromise.readFile('G:\\outx\\application\\菊姬单行本.rbq'
    ...
    .then((data)=>{
        ...
        return fsPromise.readFile('G:\\outx\\application\\汉化区2019年部分单行本.rbq'
        ,{encoding:'utf8',flag:'r'}
        )
    }).then((data)=>{
        temp2 = data.split('\n').map(item=> item.split(','))
        ...
        let fsStreamWrite = fs.createWriteStream('./重复本arr.rbq')
        for(let j of indexNum){
            fsStreamWrite.write(`${j[1]?j[1]:j}`+'\n')
        }
        fsStreamWrite.end()
